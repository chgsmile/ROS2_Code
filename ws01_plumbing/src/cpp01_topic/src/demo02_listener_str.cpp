...
public:
    Listener() : Node{"listener_node_cpp"} {
        RCLCPP_INFO(this->get_logger(), "创建订阅方");
        
        subscription_ = this->create_subscription<std_msgs::msg::String>("chatter", 10, std::bind(&Listener::on_sub, this, std::placeholders::_1));
    }

private:
    void on_sub(const std_msgs::msg::String& msg) {
            RCLCPP_INFO(this->get_logger(), "订阅到的数据是：%s", msg.data.c_str());
    }
